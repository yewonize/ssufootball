...
const AdminPlayers = ({ players, onUpdatePlayer }) => {
  // 🔥 1회용 DB 마이그레이션 함수
  const handleMigrateSeasons = async () => {
    if (
      !window.confirm(
        "모든 선수 데이터에 seasons 필드를 추가하시겠습니까? (1회만 실행하세요)",
      )
    )
      return;
    ...
    try {
      ...
      snap.docs.forEach((docSnap) => {
        ...
        const seasons = {}; // 1. 선수가 뛰었던 연도 추출 (stats.years 기반)

        const activeYears = data.stats?.years
          ? Object.keys(data.stats.years)
          : [];

        if (activeYears.length > 0) {
          activeYears.forEach((year) => {
            // 과거 연도에는 기본적으로 'current(재학)' 상태였을 것이라 가정
            seasons[year] = {
              number: data.number || "",
              status: year === currentYear ? data.status : "current",
            };
          });
        } // 2. 만약 스탯은 없지만 현재 등록된 선수라면 올해 데이터 생성

        if (!seasons[currentYear]) {
          seasons[currentYear] = {
            number: data.number || "",
            status: data.status || "current",
          };
        } // 3. 해당 선수의 문서에 seasons 맵 추가

        batch.update(docSnap.ref, { seasons });
      });

      await batch.commit();
      alert("✅ 모든 선수 데이터가 연도별 로스터 구조로 변환되었습니다!");
    } catch (error) {
      console.error(error);
  ...
  const handleDrop = async (e, newStatus) => {
    ...
    if (!playerId) return; // 데이터 변경 사항 적용
  ...
  const formatDate = (createdAt) => {
    if (!createdAt) return "날짜 정보 없음"; // Firestore Timestamp 객체인지 확인 후 처리
  ...
  return (
    <div className="space-y-6 animate-fade-in max-w-full mx-auto relative z-10">
      ...
           {" "}
      {editingPlayer && (
        <div className="fixed left-0 right-0 bottom-0 top-15 md:top-20 z-1000 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 md:p-6 animate-fade-in">
                   {" "}
          {/* max-h-full을 줘서 지정된 구역(헤더 아래)을 절대 벗어나지 않게 만듭니다 */}
                   {" "}
          <div className="bg-white rounded-4xl shadow-2xl w-full max-w-4xl flex flex-col max-h-full overflow-hidden">
                        {/* 1. 고정 헤더 */}           {" "}
      ...
      {managingCommentsPlayer && (
        <div
          className="fixed left-0 right-0 bottom-0 top-15 md:top-20 z-1000 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 md:p-6 animate-fade-in"
          onClick={() => setManagingCommentsPlayer(null)}
        >
                   {" "}
          <div
            className="bg-white rounded-4xl shadow-2xl w-full max-w-lg flex flex-col max-h-full overflow-hidden"
