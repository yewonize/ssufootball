...
import { Shield, Calendar, Users, Home, Settings, Instagram, Youtube, BookOpen } from 'lucide-react';

const Layout = ({ children, activeTab, setActiveTab, isAdmin, toggleAdmin }) => {
    ...
    <div className="min-h-dvh bg-[#f9fafb] font-sans text-gray-900 flex flex-col">
      ...
      <header className="bg-white sticky top-0 z-50 shadow-sm border-b border-gray-100">
        ...
        <div className="max-w-6xl mx-auto px-4 h-14 md:h-16 flex items-center relative">
          ...
          >
            <img src="logo.png" alt="Logo" className="h-10 md:h-20 w-auto object-contain" />
          </div>
          ...
          <div className="hidden md:flex ml-auto items-center">
             {isAdmin && (
               ...
               >
                 <Settings size={16} /><span>관리 센터</span>
               </button>
             )}
          </div>
        </div>
      ...
      <main className="flex-1 w-full pb-28 md:pb-10">
        {children}
      </main>
      ...
      <footer className="bg-ssu-dark text-white/70 py-10 pb-28 md:pb-10 w-full mt-auto">
        <div className="max-w-6xl mx-auto px-6 flex flex-col md:flex-row justify-between items-center md:items-end gap-8">
          ...
          <div className="flex flex-col items-center md:items-start text-center md:text-left">
            <h2 className="text-xl font-black text-white mb-2 tracking-tight">숭실대학교 축구단</h2>
            <p className="text-xs mb-1 font-medium">서울특별시 동작구 상도로 369 숭실대학교 대운동장</p>
          </div>

          {/* SNS 링크 (나중에 href 속성에 실제 링크를 넣으세요!) */}
          <div className="flex gap-4">
            <a href="https://instagram.com/ssu.football" target="_blank" rel="noreferrer" className="bg-white/10 p-3 rounded-full hover:bg-[#FFD60A] hover:text-ssu-dark transition-all">
              <Instagram size={20} />
            </a>
            <a href="https://youtube.com/@ssu.football" target="_blank" rel="noreferrer" className="bg-white/10 p-3 rounded-full hover:bg-[#FFD60A] hover:text-ssu-dark transition-all">
              <Youtube size={20} />
            </a>
            <a href="https://blog.naver.com/ssufootball1918" target="_blank" rel="noreferrer" className="bg-white/10 p-3 rounded-full hover:bg-[#FFD60A] hover:text-ssu-dark transition-all">
              <BookOpen size={20} />
            </a>
          </div>
        </div>
          ...
          <div className="bg-gray-200/90 backdrop-blur-xl border border-white/50 p-1.5 rounded-full flex w-full max-w-[320px] shadow-[0_10px_40px_rgba(0,0,0,0.15)] pointer-events-auto">
            {navItems.map((item) => {
              ...
              return (
                ...
                >
                  {React.cloneElement(item.icon, { size: isActive ? 20 : 18, className: isActive ? 'text-ssu-blue' : '' })}
                  <span className={`text-[10px] md:text-xs font-black tracking-wider ${isActive ? 'text-ssu-dark' : ''}`}>
                    {item.label}
                  </span>
                </button>
              );
